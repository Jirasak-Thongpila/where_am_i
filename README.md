...
erDiagram
    ...
    users {
        serial id PK
        text name "ชื่อ-นามสกุล"
        text email UK "อีเมลผู้ใช้"
        text password "Argon2 Hashed Password"
        text bio "คำอธิบายตนเอง"
        text profile_image "URL รูปโปรไฟล์ (GCS)"
        text cover_image "URL รูปหน้าปก (GCS)"
        json social_links "ลิงก์โซเชียลมีเดีย"
        boolean is_verified "สถานะยืนยันอีเมล"
    ...
    verification_tokens {
        serial id PK
        text email "อีเมลผู้รับ OTP"
        text otp "รหัส OTP 6 หลัก"
        timestamp expires_at "วันหมดอายุ (15 นาที)"
        timestamp created_at
    }

    checkins {
        serial id PK
        integer user_id FK "รหัสผู้ใช้"
        doublePrecision lat "ละติจูด"
        doublePrecision lng "ลองจิจูด"
        text location_name "ชื่อสถานที่"
        text address "ที่อยู่สถานที่"
        doublePrecision accuracy "ความแม่นยำ GPS (เมตร)"
        text description "ข้อความบันทึก"
        text image_url "URL รูปภาพเช็คอิน (GCS)"
        timestamp created_at
        timestamp updated_at
    }
```
